...
#AffineMethod=Affine(0,[0,0],0,[1,1],'constant')
AffineMethod=Affine(rotation_range=5,translation_range=[0.05,0.05],shear_range=0.05,zoom_range=[0.95,1.05])
img=torch.from_numpy(np.array(img).transpose(2,0,1))
print(img.size())
imgTensor=AffineMethod(img)
print(imgTensor.size())
#imgTensor=img    
#print(imgnp)
#haha=Image.save(,'test.jpg')
import scipy.misc
scipy.misc.imsave('test2.jpg', imgTensor.numpy().transpose(1,2,0))
'''
