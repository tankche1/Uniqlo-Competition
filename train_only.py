...
PATH_TO_MODEL = os.path.join('models', 'only',args.bh)
category_num=24
history=[0.01]*1000
historyMax=0.01
Hloss=[0.01]*1000
...
weights=torch.FloatTensor([1,1,1,1,1,1,1,1,2,3,5,3,1,5,1,2,2,3,1,2,2,1,1,1]).cuda()
...
#model.linear=nn.Linear(2048,24)
...
'''
print('go!')
grayall=torch.randn(10000,4,256,256)
tot=0
for i, (inputs, targets) in tqdm(enumerate(train_loader)):
    gray_inputs=get_glcm(inputs).float()
    #for j in range(num,num+inputs.size(0)):
    #    grayall[j]=gray_inputs[j-num]
    print(i)
    grayall[tot:tot+inputs.size(0)]=gray_inputs[0:inputs.size(0)]
...
def train(epoch):
    ...
    for i, (inputs, targets) in tqdm(enumerate(train_loader)):
        ...
        #gray_inputs[0:inputs.size(0)]=grayall[num:num+inputs.size(0)]
